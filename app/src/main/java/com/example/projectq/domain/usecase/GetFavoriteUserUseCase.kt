...
data class GetFavoriteUserUseCaseParam(val username: String)

class GetFavoriteUserUseCase @Inject constructor(
    private val repository: UserRepository,
    private val dispatcher: DispatcherProvider
) {
    fun execute(param: GetFavoriteUserUseCaseParam): Flow<UserHomeDomainModel?> {
        return repository.getFavoriteUser(param.username).flowOn(dispatcher.io)
    }
}